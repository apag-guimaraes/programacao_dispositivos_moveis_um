...
4. Bibliotecas C/C mais mais nativas: alguns componentes usam bibliotecas nativas, como a SQLite. Através do Android NDK, é possível desenvolver apps em C/C mais mais;
...
- O ciclo de vida dos componentes (Activity) é gerenciado pelo Android. O android gerencia as persmissões e segurança. Cada app roda em seu sandbox (implementada pelas permissões, onde o app define os recursos de hardware, de armazenamento que irá usar; pelo UID, ID único que define os privilégios de acesso ao sistema de arquivos e do kernel; SELinux que define as políticas de segurança em nível do kernel; No Android, os aplicativos compartilham o mesmo kernel Linux e as bibliotecas do sistema. O isolamento é feito em nível de processo e não de máquina virtual.
...
Atualmente o sistema Android encontra-se na versão 16. Para mais detalhes sobre a API e novidades, acessem os links: https://developer.android.com/about/versions/16?hl=pt-br. Acesso em: 08 de jul. 2025. 
