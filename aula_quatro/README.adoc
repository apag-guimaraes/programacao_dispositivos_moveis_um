...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener {
    ...
    private void preencherAdapter() {
        ArrayList<String> listaLinguagens = (ArrayList<String>) RepositorioLinguagens.getChavesLinguagens();
...
public class RepositorioLinguagens {
    ...
    public static Map<String,Integer> getMap(){
        return linguagemCaracteristicas;
    }
    public static List<Integer> getValoresLinguagens() {
        return new ArrayList<>(linguagemCaracteristicas.values());
    }
    public static List<String> getChavesLinguagens() {
        return new ArrayList<>(linguagemCaracteristicas.keySet());
    }
    public static String getCaracteristicas(String linguagemSelecionada, Resources resources) {
        String chave = linguagemSelecionada.toUpperCase();
        Integer arrayResId = linguagemCaracteristicas.get(chave);
        String[] caracteristicas = resources.getStringArray(arrayResId);
        StringBuilder sb = new StringBuilder();
        for (String caracteristica : caracteristicas) {
            sb.append("--> ").append(caracteristica.trim()).append("\n");
        }
        return sb.toString();
    }
}
...
public class SegundaActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String texto = RepositorioLinguagens.getCaracteristicas(linguagemSelecionada,getApplicationContext().getResources());
...
class MainActivity : AppCompatActivity() {
    private lateinit var listViewLinguagens: ListView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listViewLinguagens = findViewById(R.id.listViewLinguagens)
        preencherAdapter()
        listViewLinguagens.adapter = adapter
        listViewLinguagens.setOnItemClickListener { adapterView, _, position, _ ->
            val item = adapterView.getItemAtPosition(position).toString()
            abrirTela(item)
        }
        listViewLinguagens.setOnItemLongClickListener { adapterView, _, position, _ ->
            val linguagem = adapterView.getItemAtPosition(position).toString()
            Toast.makeText(this, linguagem, Toast.LENGTH_SHORT).show()
            true // Retorna true indicando que o clique longo foi consumido
        }
    }
    private fun preencherAdapter() {
        val listaLinguagens = RepositorioLinguagens.getChavesLinguagens() as ArrayList<String>
        adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, listaLinguagens)
    }
    fun abrirTela(item: String) {
        when (item) {
            "C", "JAVA" -> {
                val intent = Intent(this, SegundaActivity::class.java).apply {
                    putExtra("linguagem", item)
                }
                startActivity(intent)
            }
            else -> {
...
- RepositorioLinguagens.kt
[source,kotlin]
import android.content.res.Resources
class RepositorioLinguagens {
    companion object {
        // Inicialização limpa e direta do Map estático
        private val linguagemCaracteristicas: Map<String, Int> = mapOf(
            "C" to R.array.c,
            "JAVA" to R.array.java,
            "KOTLIN" to R.array.kotlin,
            "LUA" to R.array.lua,
            "PASCAL" to R.array.pascal,
            "PYTHON" to R.array.python
        )
        fun getMap(): Map<String, Int> = linguagemCaracteristicas
        fun getValoresLinguagens(): List<Int> {
            return ArrayList(linguagemCaracteristicas.values)
        }
        fun getChavesLinguagens(): List<String> {
            return ArrayList(linguagemCaracteristicas.keys)
        }
        fun getCaracteristicas(linguagemSelecionada: String, resources: Resources): String {
            val chave = linguagemSelecionada.uppercase()
            val arrayResId = linguagemCaracteristicas[chave] ?: return "Linguagem não encontrada"
            val caracteristicas = resources.getStringArray(arrayResId)
            return caracteristicas.joinToString(separator = "") { caracteristica ->
                "--> ${caracteristica.trim()}\n"
            }
        }
    }
...
class SegundaActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_segunda)
        val linguagemSelecionada = intent.getStringExtra("linguagem") ?: ""
        val texto = RepositorioLinguagens.getCaracteristicas(linguagemSelecionada, resources)
        val textViewCaracteristicas = findViewById<TextView>(R.id.textViewCaracteristicas)
        textViewCaracteristicas.text = texto
    }
}
...
public class RepositorioItens {
    public static List<ItemLista> obterItens() {
...
- ItemLista.kt
[source,kotlin]
data class ItemLista(var nome: String, var imagem: Int)

- RepositorioItens.kt
[source,kotlin]
class RepositorioItens {
    companion object {
        fun obterItens(): List<ItemLista> {
            return listOf(
                ItemLista("item 1", R.drawable.ic_launcher_foreground),
                ItemLista("item 2", R.drawable.ic_launcher_foreground),
                ItemLista("item 3", R.drawable.ic_launcher_foreground)
            )
        }
    }
}
...
class MeuAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        val texto = view.findViewById<TextView>(R.id.textoViewItens)
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_main)
        listView = findViewById(R.id.listViewItens)
        val itens = RepositorioItens.obterItens()
        meuAdapter = MeuAdapter(this, itens)
        listView.adapter = meuAdapter
        listView.setOnItemClickListener { parent, _, position, _ ->
            val item = parent.getItemAtPosition(position) as ItemLista
            Toast.makeText(this, "Você Clicou em: ${item.nome}", Toast.LENGTH_LONG).show()
        }
    }
}
