...
Disponível em: https://developer.android.com/training/data-storage?hl=pt-br. Acesso em: 16 ago. 2025.
...
import java.io.FileOutputStream
import java.io.IOException
class ManipulaArquivo(private val context: Context) {
    @Throws(IOException::class)
    fun salvarArquivo(nome: String, conteudo: String, append: Boolean) {
        val arquivo = File(context.filesDir, nome)
        FileOutputStream(arquivo, append).use { fos ->
            fos.write(conteudo.toByteArray())
        }
    }
    @Throws(IOException::class)
    fun lerArquivo(nome: String): String {
        return context.openFileInput(nome).bufferedReader().use { reader ->
            reader.readText()
        }
    }
    fun deletarArquivo(nome: String): Boolean {
        return context.deleteFile(nome)
    }
}
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var arquivo: ManipulaArquivo
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonSalvar = findViewById(R.id.buttonSalvar)
        buttonExcluir = findViewById(R.id.buttonExcluir)
        arquivo = ManipulaArquivo(this)
        intent?.getStringExtra("texto")?.let { texto ->
            editTextTexto.setText(texto)
        }
    ...
    private fun salvarTexto() {
        val conteudo = editTextTexto.text.toString()
        try {
            arquivo.salvarArquivo("meuArq.txt", conteudo, true)
            Toast.makeText(this, "Texto salvo com sucesso", Toast.LENGTH_SHORT).show()
            editTextTexto.setText("")
...
class SegundaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arquivo = ManipulaArquivo(this)
        try {
            val texto = arquivo.lerArquivo("meuArq.txt")
            textViewTexto.text = texto
        } catch (e: IOException) {
            e.printStackTrace()
            textViewTexto.text = "Erro ao carregar conteúdo"
            Toast.makeText(this, "Erro ao ler o arquivo", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun voltarTela() {
        val intent = Intent(this, MainActivity::class.java).apply {
            putExtra("texto", textViewTexto.text.toString())
        }
        startActivity(intent)
        finish()
    }
...
import java.io.IOException
class ManipulaArquivo(private val context: Context) {
    private fun getArquivo(nome: String): File {
        val dir = context.getExternalFilesDir(null)
        return File(dir, nome)
    }
    @Throws(IOException::class)
    fun salvarArquivo(nome: String, conteudo: String) {
        val arquivo = getArquivo(nome)
        FileOutputStream(arquivo, true).use { fos ->
            fos.write(conteudo.toByteArray())
        }
    }
    @Throws(IOException::class)
    fun lerArquivo(nome: String): String {
        val arquivo = getArquivo(nome)
        return FileInputStream(arquivo).bufferedReader().use { reader ->
            reader.readText()
        }
    }
    fun deletarArquivo(nome: String): Boolean {
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)
        editTextTexto = findViewById(R.id.editTextNome)
        buttonSalvar = findViewById(R.id.buttonS)
        buttonExcluir = findViewById(R.id.buttonE)
        arquivo = ManipulaArquivo(this)
        intent?.getStringExtra("texto")?.let { texto ->
            editTextTexto.setText(texto)
        }
...
class ManipulaArquivo {
    companion object {
        ...
        private const val NOME_ARQUIVO = "meuArq.txt"
        fun salvarDados(context: Context, data: String): Boolean {
            try {
                ...
                uri?.let { validUri ->
                    context.contentResolver.openOutputStream(validUri).use { outputStream ->
                        outputStream?.write(data.toByteArray())
                        return true
                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Erro ao salvar o arquivo no diretório Downloads", e)
            }
            return false
        }
        fun lerDados(context: Context): String {
            val dados = StringBuilder()
            val uri = MediaStore.Files.getContentUri("external")
            val selecao = "${MediaStore.MediaColumns.DISPLAY_NAME} = ?"
            val argumentos = arrayOf(NOME_ARQUIVO)
            try {
                context.contentResolver.query(uri, null, selecao, argumentos, null).use { cursor ->
                    if (cursor != null && cursor.moveToFirst()) {
                        val idColuna = cursor.getColumnIndex(MediaStore.MediaColumns._ID)
                        if (idColuna != -1) {
                            val idArquivo = cursor.getLong(idColuna)
                            val uriArquivo = ContentUris.withAppendedId(
                                MediaStore.Files.getContentUri("external"), 
                                idArquivo
                            )
                            context.contentResolver.openInputStream(uriArquivo).use { inputStream ->
                                inputStream?.bufferedReader()?.use { reader ->
                                    dados.append(reader.readText())
                                }
                            }
                        }
                    }
                ...
                Log.e(TAG, "Erro ao ler o arquivo", e)
            }
            return dados.toString()
        }
        fun deletarDados(context: Context): Boolean {
            val uri = MediaStore.Files.getContentUri("external")
            val selecao = "${MediaStore.MediaColumns.DISPLAY_NAME} = ?"
            val argumentos = arrayOf(NOME_ARQUIVO)
            try {
                val linhasDeletadas = context.contentResolver.delete(uri, selecao, argumentos)
                return linhasDeletadas > 0
            } catch (e: Exception) {
                Log.e(TAG, "Erro ao excluir o arquivo", e)
            }
            return false
        }
    }
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent?.getStringExtra("msg")?.let { msg ->
            editTextNome.setText(msg)
        }
    }
    fun clicar(view: View) {
        when (view.id) {
            R.id.buttonS -> {
                val dado = editTextNome.text.toString()
                if (ManipulaArquivo.salvarDados(applicationContext, dado)) {
            ...
            R.id.buttonE -> {
                if (ManipulaArquivo.deletarDados(applicationContext)) {
                    Toast.makeText(applicationContext, "Arquivo excluído", Toast.LENGTH_SHORT).show()
                    editTextNome.setText("")
                } else {
...
class SegundaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_segunda)
        textView = findViewById(R.id.textView)
        val data = ManipulaArquivo.lerDados(applicationContext)
        textView.text = data
    }
}
