...
import android.database.sqlite.SQLiteDatabase
import android.database.sqlite.SQLiteOpenHelper
class DBHelper(context: Context?) : SQLiteOpenHelper(context, BANCO_NOME, null, BANCO_VERSAO) {
...
class DatabaseManager(context: Context) {
    ...
    fun adicionar(p: Pessoa): Long {
        val values = ContentValues().apply {
            put(DBHelper.NOME, p.nome)
            put(DBHelper.TELEFONE, p.telefone)
        }
        return database.insert(DBHelper.BANCO_TABELA, null, values)
    }

    fun listar(): ArrayList<Pessoa> {
        val dados = ArrayList<Pessoa>()
        val consulta = "SELECT * FROM ${DBHelper.BANCO_TABELA}"
        database.rawQuery(consulta, null).use { cursor ->
            while (cursor.moveToNext()) {
                val p = Pessoa(
                    id = cursor.getInt(0),
    ...
    fun atualizar(p: Pessoa): Int {
        val values = ContentValues().apply {
            put(DBHelper.NOME, p.nome)
            put(DBHelper.TELEFONE, p.telefone)
        }
        val where = "${DBHelper.ID}=?"
        val args = arrayOf(p.id.toString())
        return database.update(DBHelper.BANCO_TABELA, values, where, args)
    }
    fun excluir(p: Pessoa): Int {
        val where = "${DBHelper.ID}=?"
        val args = arrayOf(p.id.toString())
        return database.delete(DBHelper.BANCO_TABELA, where, args)
    }
        ...
        when (view.id) {
            R.id.buttonS -> {
                ...
                if (currentPessoa == null) {
                    ...
                    val retorno = databaseManager.adicionar(novaPessoa)
                    if (retorno != -1L) {
                    ...
                    currentPessoa.nome = editTextNome.text.toString()
                    currentPessoa.telefone = editTextTelefone.text.toString()
                    val retorno = databaseManager.atualizar(currentPessoa)
                    if (retorno != 0) {
            ...
            R.id.buttonL -> {
                val dados = databaseManager.listar()
                Log.i("dados", dados.toString())
                if (dados.isNotEmpty()) {
            ...
            R.id.buttonD -> {
                val currentPessoa = p
                if (currentPessoa != null) {
                    val retorno = databaseManager.excluir(currentPessoa)
                    if (retorno > 0) {
                    ...
                    Toast.makeText(this, "Nenhum registro selecionado", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
...
class SegundaActivity : AppCompatActivity(), AdapterView.OnItemClickListener {
    ...
    override fun onItemClick(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        val p = parent?.getItemAtPosition(position) as? Pessoa
        p?.let {
...
class UserDAO(private val db: SQLiteDatabase?) {
    fun insertUser(name: String, age: Int): Long {
        db?.let {
            val values = ContentValues().apply {
                put(UserContract.COLUMN_NAME, name)
                put(UserContract.COLUMN_AGE, age)
            }
            return it.insert(UserContract.TABLE_NAME, null, values)
        }
        return -1
    }
    fun getAll(): List<User> {
        ...
        db?.let {
            ...
            it.query(
                ...
                while (cursor.moveToNext()) {
                    val idUser = cursor.getLong(cursor.getColumnIndexOrThrow(UserContract._ID))
                    val nameUser = cursor.getString(cursor.getColumnIndexOrThrow(UserContract.COLUMN_NAME))
                    val ageUser = cursor.getInt(cursor.getColumnIndexOrThrow(UserContract.COLUMN_AGE))
                    users.add(User(idUser, nameUser, ageUser))
                }
            }
        }
        return users
...
class MainActivity : AppCompatActivity() {
    ...
    fun clicar(view: View) {
        when (view.id) {
            R.id.buttonInsert -> {
                val userId1 = userDAO.insertUser("Ana", 30)
                val userId2 = userDAO.insertUser("Rodrigo", 20)
                val result = "Inserted IDs: $userId1 - $userId2"
                textViewResult.text = result
            }
            R.id.buttonList -> {
                val dados = userDAO.getAll()
                textViewResult.text = dados.toString()
            }
        }
    }
    override fun onDestroy() {
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clicar(View view) {
        if (view.getId() == R.id.buttonInserir) {
            ...
            long aluno3 = alunoDAO.insert("João", mat1); 
...
data class Aluno(
    ...
    override fun toString(): String {
        return "Aluno{id=$id, nome='$nome', disciplinaId=$disciplinaId}"
    }
}
...
class DBManager(context: Context) {
    ...
    var database: SQLiteDatabase? = null
        private set
    fun open() {
        database = dbHelper.writableDatabase.apply {
...
import android.content.ContentValues
import android.database.sqlite.SQLiteDatabase
class AlunoDAO(private val db: SQLiteDatabase?) {
    fun insert(nome: String, disciplinaId: Long): Long {
        db?.let {
            val values = ContentValues().apply {
                put("nome", nome)
                put("disciplina_id", disciplinaId)
            }
            return it.insert("aluno", null, values)
        }
        return -1
    }
    fun getAll(): List<Aluno> {
        val list = mutableListOf<Aluno>()
        db?.query("aluno", null, null, null, null, null, null)?.use { cursor ->
            while (cursor.moveToNext()) {
                val id = cursor.getLong(cursor.getColumnIndexOrThrow("id"))
                val nome = cursor.getString(cursor.getColumnIndexOrThrow("nome"))
                val disciplinaId = cursor.getLong(cursor.getColumnIndexOrThrow("disciplina_id"))
                list.add(Aluno(id, nome, disciplinaId))
            }
        }
        return list
    }
...
import android.content.ContentValues
import android.database.sqlite.SQLiteDatabase
class DisciplinaDAO(private val db: SQLiteDatabase?) {
    fun insert(nome: String): Long {
        db?.let {
            val values = ContentValues().apply {
                put("nome", nome)
            }
            return it.insert("disciplina", null, values)
        }
        return -1
    }
    fun getAll(): List<Disciplina> {
        val list = mutableListOf<Disciplina>()
        db?.query("disciplina", null, null, null, null, null, null)?.use { cursor ->
            while (cursor.moveToNext()) {
                val id = cursor.getLong(cursor.getColumnIndexOrThrow("id"))
                val nome = cursor.getString(cursor.getColumnIndexOrThrow("nome"))
                list.add(Disciplina(id, nome))
            }
        }
        return list
    }
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)
        dbManager = DBManager(this).apply { open() }
        alunoDAO = AlunoDAO(dbManager.database)
        disciplinaDAO = DisciplinaDAO(dbManager.database)
        btnInsert = findViewById(R.id.buttonInserir)
        btnList = findViewById(R.id.buttonListar)
        textViewResult = findViewById(R.id.textViewResult)
    }
    fun clicar(view: View) {
        when (view.id) {
            R.id.buttonInserir -> {
                val mat1 = disciplinaDAO.insert("Matemática")
                val hist1 = disciplinaDAO.insert("História")
                val aluno1 = alunoDAO.insert("Ana", mat1)
                val aluno2 = alunoDAO.insert("João", hist1)
                val aluno3 = alunoDAO.insert("João", mat1) 
            ...
            R.id.buttonListar -> {
                ...
                val disciplinas = disciplinaDAO.getAll()
                textViewResult.text = "Alunos: $alunos\nDisciplinas: $disciplines"
            }
        }
    }
    override fun onDestroy() {
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clicar(View view) {
        if (view.getId() == R.id.buttonInserir) {
            ...
            alunoDisciplinaDAO.linkAlunoDisciplina(aluno1, disc2); 
...
object DatabaseContract {
    object AlunoEntry : BaseColumns {
        const val TABLE_NAME = "aluno"
        const val COLUMN_NOME = "nome"
        const val COLUMN_IDADE = "idade"
        const val _ID = BaseColumns._ID
    }
    object DisciplinaEntry : BaseColumns {
        const val TABLE_NAME = "disciplina"
        const val COLUMN_NOME = "nome"
        const val _ID = BaseColumns._ID
    }
    object AlunoDisciplinaEntry : BaseColumns {
        const val TABLE_NAME = "aluno_disciplina"
        const val COLUMN_ALUNO_ID = "aluno_id"
        const val COLUMN_DISCIPLINA_ID = "disciplina_id"
        const val _ID = BaseColumns._ID
    }
}
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL(
            ...
            CREATE TABLE ${DatabaseContract.AlunoDisciplinaEntry.TABLE_NAME} (
                ...
                ${DatabaseContract.AlunoDisciplinaEntry.COLUMN_ALUNO_ID} INTEGER,
                ${DatabaseContract.AlunoDisciplinaEntry.COLUMN_DISCIPLINA_ID} INTEGER,
                UNIQUE(${DatabaseContract.AlunoDisciplinaEntry.COLUMN_ALUNO_ID}, ${DatabaseContract.AlunoDisciplinaEntry.COLUMN_DISCIPLINA_ID}),
                FOREIGN KEY(${DatabaseContract.AlunoDisciplinaEntry.COLUMN_ALUNO_ID}) REFERENCES ${DatabaseContract.AlunoEntry.TABLE_NAME}(${DatabaseContract.AlunoEntry._ID}) ON DELETE CASCADE,
                FOREIGN KEY(${DatabaseContract.AlunoDisciplinaEntry.COLUMN_DISCIPLINA_ID}) REFERENCES ${DatabaseContract.DisciplinaEntry.TABLE_NAME}(${DatabaseContract.DisciplinaEntry._ID}) ON DELETE CASCADE
            )
            """.trimIndent()
        )
    }
...
import android.database.sqlite.SQLiteDatabase
class AlunoDAO(private val db: SQLiteDatabase?) {
    fun insert(nome: String, idade: Int): Long {
        db?.let {
            val values = ContentValues().apply {
                put("nome", nome)
                put("idade", idade)
...
- AlunoDisciplinaDAO.kt
[source,kotlin]
import android.content.ContentValues
import android.database.sqlite.SQLiteDatabase
class AlunoDisciplinaDAO(private val db: SQLiteDatabase?) {
    fun linkAlunoDisciplina(alunoId: Long, disciplinaId: Long): Long {
        db?.let {
            val values = ContentValues().apply {
                put("aluno_id", alunoId)
                put("disciplina_id", disciplinaId)
            }
            return it.insert("aluno_disciplina", null, values)
        }
        return -1
    }
    fun getAlunosDisciplinas(): List<String> {
        ...
        db?.rawQuery(query, null)?.use { cursor ->
            while (cursor.moveToNext()) {
                val aluno = cursor.getString(0)
                val disciplina = cursor.getString(1)
                list.add("$aluno -> $disciplina")
            }
        }
        return list
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var alunoDAO: AlunoDAO
    private lateinit var disciplinaDAO: DisciplinaDAO
    private lateinit var alunoDisciplinaDAO: AlunoDisciplinaDAO
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        alunoDAO = AlunoDAO(dbManager.database)
        disciplinaDAO = DisciplinaDAO(dbManager.database)
        alunoDisciplinaDAO = AlunoDisciplinaDAO(dbManager.database)
        btnInserir = findViewById(R.id.buttonInserir)
        btnListar = findViewById(R.id.buttonListar)
        textViewResult = findViewById(R.id.textViewResult)
    }
    fun clicar(view: View) {
        when (view.id) {
            R.id.buttonInserir -> {
                val aluno1 = alunoDAO.insert("Ana", 20)
                val aluno2 = alunoDAO.insert("João", 22)
                val disc1 = disciplinaDAO.insert("Matemática")
                val disc2 = disciplinaDAO.insert("História")
                alunoDisciplinaDAO.linkAlunoDisciplina(aluno1, disc1)
                alunoDisciplinaDAO.linkAlunoDisciplina(aluno1, disc2) 
                alunoDisciplinaDAO.linkAlunoDisciplina(aluno2, disc1)
                textViewResult.text = "Inserido com sucesso!"
            }
            R.id.buttonListar -> {
                val list = alunoDisciplinaDAO.getAlunosDisciplinas()
                // Une toda a lista quebrando linhas de forma direta e idiomática
                textViewResult.text = list.joinToString("\n")
            }
        }
    }
    override fun onDestroy() {
