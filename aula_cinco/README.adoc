...
A barra de ações é um componente fundamental em aplicativos, servindo tanto para identificar a aplicação quanto para oferecer funcionalidades adicionais, como o menu flutuante, que proporciona opções extras ao usuário. Existem duas classes principais para implementar a barra de ações no Android:
